# modules/features/server/borg-backup.nix
{ self, ... }:
let
  consts = import ../../_consts.nix;
in
{
  flake.nixosModules.borgBackup =
    { pkgs, ... }:
    let
      ssdPushUrl = "http://localhost:${consts.ports.uptimeKuma}/api/push/7A0I8FgFnTB40XNRkEG93OfpxOVhKHow";
      piPushUrl = "http://localhost:${consts.ports.uptimeKuma}/api/push/P24iryj91s7X4bN57ZvVrbMzA2prL7b1";
    in
    {
